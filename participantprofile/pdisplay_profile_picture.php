...
if (isset($_POST['new_username'])) {
    ...
    $query = "UPDATE participant SET Username = :newUsername WHERE Participant_Id = :userID";
...
if (isset($_FILES['new_avatar'])) {
    ...
    if (in_array($fileType, $allowedTypes)) {
        ...
        $query = "UPDATE participant SET profile_picture = ? WHERE Participant_Id = ?";
        $stmt = $con->prepare($query);
...
$query = "SELECT profile_picture FROM participant WHERE Participant_Id = :id";
$stmt = $con->prepare($query);
