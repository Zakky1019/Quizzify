...
$edit = $_GET['edit'];
$sql = "SELECT * FROM participant WHERE Participant_Id = '$edit'";
...
if(isset($_POST['submit']))
{
    ...
    $stmt = $connection->prepare("UPDATE participant SET FirstName=?, LastName=?,Username=?, Email=?, Password=? WHERE Participant_Id=?");
    
    // Bind parameters
    $stmt->bind_param("sssssi", $First_Name, $Last_Name, $Username, $Email, $Password, $edit);
    
    if ($stmt->execute()) {
        echo '<script>location.replace("participant1.php")</script>';
    } else {
...
<head>
    ...
    <title>Edit paticipant Details</title>
            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-header"> 
                        <h1>Edit participant Details</h1>
                    </div>
                    <div class="card-body">
