...
if (isset($_POST['new_username'])) {
    ...
    $query = "UPDATE instructor SET Username = :newUsername WHERE Instructor_Id = :userID";
...
if (isset($_FILES['new_avatar'])) {
    ...
    if (in_array($fileType, $allowedTypes)) {
        ...
        $query = "UPDATE instructor SET profile_picture = ? WHERE Instructor_Id = ?";
        $stmt = $con->prepare($query);
        ...
        header("Location: iuser_profile.php");
        exit();
    } else {
...
$query = "SELECT profile_picture FROM instructor WHERE Instructor_Id = :id";
$stmt = $con->prepare($query);
